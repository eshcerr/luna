...
ui_element_process_inputs :: proc(ctx: ^ui_context_t, elem: ^ui_element_t, inputs: ^core.input_t) {
	...
	case button_t:
		elem.element.(button_t).state = button_state_e(
			i32(core.iaabb_contains(elem.bounding_box, ctx.mouse_pos)),
		)
		if (elem.element.(button_t).state == .HOVERED) {
			elem.element.(button_t).state += button_state_e(
				i32(core.inputs_mouse_button_down(.MOUSE_BUTTON_1)),
			)
		}
	case toggle_t:
		elem.element.(toggle_t).state = button_state_e(
			i32(core.iaabb_contains(elem.bounding_box, ctx.mouse_pos)),
		)
